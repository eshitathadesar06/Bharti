...
# ---------------- ATTENDANCE ----------------
    ...
    is_holiday = not holidays_df[
        (holidays_df["date"]==date_str) & (holidays_df["batch"]==batch)
    ].empty
    ...
    with col_h1:
        if st.button("Mark as Holiday"):
            if is_holiday:
                st.warning(f"{date_str} is already marked as holiday for {batch} batch")
            else:
                new_holiday = pd.DataFrame([[date_str, batch]], columns=["date","batch"])
                holidays_df = pd.concat([holidays_df, new_holiday], ignore_index=True)
                holidays_df.to_csv(holidays_file, index=False)
                st.success(f"{date_str} marked as holiday for {batch} batch")
                st.rerun()
    with col_h2:
        if st.button("Remove Holiday"):
            if is_holiday:
                ...
                holidays_df.to_csv(holidays_file, index=False)
                st.success(f"Holiday removed for {date_str} batch {batch}")
                st.rerun()
            else:
                st.warning("No holiday set for this day and batch")

    if is_holiday:
        st.info(f"{date_str} is a holiday for {batch} batch. Attendance cannot be modified.")
    else:
        ...
        if batch_students.empty:
            ...
            for _, row in batch_students.iterrows():
                ...
                if st.button(f"Save Attendance for {name}", key=f"save_{sid}_{date_str}"):
                    ...
                    new_row = pd.DataFrame([[date_str, sid, new_status]], columns=["date","student_id","status"])
            ...
            for _, s in batch_students.iterrows():
                ...
                present_days = len(attendance_df[
                    (attendance_df["student_id"]==sid) & (attendance_df["status"]=="Present")
                ])
                percent = (present_days/total_days*100) if total_days>0 else 0
                percentages.append([s["name"], percent])

            perc_df = pd.DataFrame(percentages, columns=["Student","Attendance %"])
...
# ---------------- FEES ----------------
    ...
    if students_df.empty:
        ...
        display_fees = fees_df.copy()
        if search:
            search_lower = search.lower()
            display_fees = display_fees.merge(students_df[["id","name"]], left_on="student_id", right_on="id", how="left")
            display_fees = display_fees[
                display_fees.apply(lambda x: search_lower in str(x["name"]).lower() or search_lower in str(x["month"]).lower(), axis=1)
            ]
        ...
        if display_fees.empty:
            ...
            display_fees = display_fees.merge(students_df[["id","name"]], left_on="student_id", right_on="id", how="left")
            display_fees = display_fees.rename(columns={
                "name":"Student Name",
                "date":"Payment Date",
                "month":"Fee Month",
                "amount":"Amount",
